...
export function ThreadView({ messages, onApprove, onDecline, onLoadDiff, onSyncComments, onUpdatePr, onApprovePlan, onRejectPlan }: ThreadViewProps) {
  ...
  return (
    <div className="flex-1 overflow-y-auto py-3">
      ...
      {messages.map((m) => <MessageItem key={m.id} message={m} onApprove={onApprove} onDecline={onDecline} onLoadDiff={onLoadDiff} onSyncComments={onSyncComments} onUpdatePr={onUpdatePr} onApprovePlan={onApprovePlan} onRejectPlan={onRejectPlan} />)}
      <div ref={endRef} />
    </div>
  );
}
