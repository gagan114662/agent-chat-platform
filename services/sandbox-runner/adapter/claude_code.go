...
func runAgentShared(ctx context.Context, exec execFunc, model, provider string, mcpServers []string, repoDir, prompt, startMsg, doneMsg, failMsg string, emit Emit) error {
	...
	mcpConfig, cleanupMcp, err := provisionMcpConfig(repoDir, mcpServers)
	if err == nil {
		defer cleanupMcp()
	}
...
// (validated) model is selected and --mcp-config when an MCP config was
// provisioned. mode is the --permission-mode value.
func claudeArgs(intent, model, mode, mcpConfig string) []string {
